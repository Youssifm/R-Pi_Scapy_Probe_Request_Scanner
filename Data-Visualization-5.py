...
class Data_Processing():
    ...
    def KNN(self):
        ...
        # SQL CMD get all unique MAC Addresses in the specified time window
        sql_cmd = """SELECT distinct(mac) FROM scapy_drli WHERE date_time between '{}' and '{}';""".format(
            t_begin.strftime('%Y-%m-%d %H:%M:%S'),
            t_end.strftime('%Y-%m-%d %H:%M:%S')
        )
        mac_temp = sql.db_read(sql_cmd)


        # Create list of probe request within the specified time window for each unique MAC address 
        for mac in mac_temp:
            vt = []
            sql_cmd = """SELECT * FROM scapy_drli WHERE mac = '{}' and date_time between '{}' and '{}';""".format(
                mac[0],
            ...
            for time_segment in range(0, int((t_end - t_begin).seconds / 20)):
                ...
                if t_temp[0] != [] and t_temp[1] != [] and t_temp[2] != []:
                    vt.append([windowS, [np.average(t_temp[0]), np.average(t_temp[1]), np.average(t_temp[2])]])
            ...
            for value in vt:
                ...
                for l in Cal_List:
                    ...
                    distance_from_Cal[len(distance_from_Cal) - 1].append(l[0])
                    for cal_data_point in l[1]:
                        distance_from_Cal[len(distance_from_Cal)-1].append(
                            math.sqrt(
                                math.pow((cal_data_point[0] - value[1][0]), 2) +
                                math.pow((cal_data_point[1] - value[1][1]), 2) +
                                math.pow((cal_data_point[2] - value[1][2]), 2)
                            )
                        )

                temp_avg = ["temp_avg", 200]

                # print "Averages for Distance Matrix\n-------------------------------------"
                for dList in distance_from_Cal:
                    #print "dlist 1 = {} ---- dlist 0 = {} ---- temp_avg = {}".format(dList[1], dList[0], temp_avg)
                    if numpy.mean(dList[1]) < numpy.mean(temp_avg[1]):
                        temp_avg = dList
                        #print "here 1----------"

                # print temp_avg
                if temp_avg != 200:
                    print "{}. {} in {}".format(mac_temp.index(mac) + 1, mac[0], temp_avg[0])
